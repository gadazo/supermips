...
void forwardUnit(SIM_cmd* pCurCmd, int &dstIndex, int32_t& dstValue,
		bool& dstValid, bool &isStall, int curMemDst, int curExeDst);
//************************** Global Variables
dataStruct dataReg;
SIM_coreState prevState;
SIM_coreState nextRegisters;
int32_t pcState[SIM_PIPELINE_DEPTH - 3];
bool prevStallDec;
bool prevStallMem;
bool prevBranch;
bool memForward;
	...
	try {
		...
		if (prevStallMem) {
			...
			prevState.pipeStageState[WRITEBACK] =
					prevState.pipeStageState[MEMORY];
			prevState.pipeStageState[MEMORY] =
					prevState.pipeStageState[EXECUTE];
			prevState.pipeStageState[EXECUTE] = *nopState;
			...
			//PC+4
			SIM_cmd_opcode CurOpcode =
					prevState.pipeStageState[FETCH].cmd.opcode;
			...
			prevState.pipeStageState[MEMORY] =
					prevState.pipeStageState[EXECUTE];
			prevState.pipeStageState[EXECUTE] =
					prevState.pipeStageState[DECODE];
			prevState.pipeStageState[DECODE] =
					prevState.pipeStageState[FETCH];
			prevState.pc = nextRegisters.pc;
...
void decodeStage(bool &isStall, dataStruct &nextData) {
	...
	forwardUnit(&prevState.pipeStageState[MEMORY].cmd, pCurCmd->src1,
			fU[0].src1_value, fU[0].src1_valid, isStall, curMemDst, curExeDst);
	//searching for src2
	...
	//  ~second - check if memory load it now
		...
		forwardUnit(&prevState.pipeStageState[MEMORY].cmd, pCurCmd->src2,
				fU[0].src2_value, fU[0].src2_valid, isStall, curMemDst,
				curExeDst);

	}
	//searching for dst
	if ((pCurCmd->opcode == CMD_STORE) || (pCurCmd->opcode == CMD_BR)
			|| (pCurCmd->opcode == CMD_BREQ)
			|| (pCurCmd->opcode == CMD_BRNEQ)) {
		forwardUnit(&prevState.pipeStageState[MEMORY].cmd, pCurCmd->dst,
				nextData.dst_value, isDstValid, isStall, curMemDst, curExeDst);
	}
	return;
}
...
void forwardUnit(SIM_cmd* pCurCmd, int &dstIndex, int32_t& dstValue,
	...
	} else if ((exeCmd->dst == dstIndex)
			&& ((exeCmd->opcode != CMD_BR) && (exeCmd->opcode != CMD_BRNEQ)
					&& (exeCmd->opcode != CMD_BREQ)
					&& (exeCmd->opcode != CMD_NOP))) {
		isStall = true;
	}
...
void memoryStage(bool &isStall, bool &isBranch, dataStruct &nextData) {
	...
	if (memForward) {
		if (cmd == CMD_ADD || cmd == CMD_SUB || cmd == CMD_ADDI
				|| cmd == CMD_SUBI) { //it's an assert, but I don't want to add real asserts
