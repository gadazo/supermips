...
void decodeStage(bool &isStall , int* pDstValDec ){
  ...
  if((pCurCmd->opcode != CMD_BR) || (pCurCmd->opcode != CMD_HALT) || (pCurCmd->opcode != CMD_NOP)){
  ...
  if((pCurCmd->opcode == CMD_BR) || (pCurCmd->opcode == CMD_BREQ) || (pCurCmd->opcode == CMD_BRNEQ)){ //retrieving dst Value
    pDstValDec[0] = 1;
    if (pForUn->validDst)
